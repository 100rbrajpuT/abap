*&---------------------------------------------------------------------*
*& Report ZPP_PRODUCTION_ORDER_RPT
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZPP_PRODUCTION_ORDER_RPT.

include zpp_production_top .
...
PARAMETERS: p_plant    TYPE werks_d DEFAULT '2304',
            p_frdate TYPE datum DEFAULT '20230401',
            p_todate   TYPE datum DEFAULT '20241231',
            p_matnr    TYPE matnr . "DEFAULT  'AQ_AFR'.
...
*    afa~iedd AS finish_date,
  ...
  LEFT JOIN ZI_BATCHCHARINFO AS z ON a~charg = z~charg
  ...
  WHERE a~bwart IN ('261','262','531','532','101','102')
    AND a~werks = @p_plant
    AND a~bldat BETWEEN @p_frdate AND @p_todate
    AND (
*   /* p_matnr IS INITIAL
*    OR
     p~matnr = @p_matnr )
