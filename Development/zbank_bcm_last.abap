...
*      lt_output    TYPE TABLE OF tline,
*      lv_job_output TYPE tsformoutput.
**************** INPUT SCREEN ******************************************
...
*                 s_batch  FOR bnk_batch_header-Batch_no ,   "batch
*                 s_LAUF FOR bnk_batch_header-laufd.         "date
...
*for transactions / bill
  ...
  LOOP AT it_item INTO wa_item .
    LOOP AT it_acdoca INTO wa_acdoca WHERE augbl = wa_item-vblnr. " belnr = wa_item-vblnr. "
      "20-12-2024
      IF wa_acdoca-augbl IS INITIAL.
         MESSAGE 'Field AUGBL is empty, skipping this record.' TYPE 'I'. " Informational message
        " CONTINUE. " Skip the iteration if augbl is empty
      ENDIF.
...
*       IF wa_acdoca-blart IN 'AB''RE''KZ' 'ZE' 'KR' .
      IF wa_acdoca-blart = 'AB' OR wa_acdoca-blart = 'RE' OR wa_acdoca-blart = 'KZ' OR wa_acdoca-blart = 'ZE' OR wa_acdoca-blart = 'KR'
        ...
        " Adjust wrbtr if blart = 'AB'
        IF wa_acdoca-blart = 'AB'.
          wa_bill-wrbtr = wa_bill-wrbtr * -1.
          wa_bill-skfbt = wa_bill-wrbtr * 1.
        ENDIF.
...
*data: lv_cd TYPE bukrs,
*      lv_comp TYPE string.
*
*
*READ TABLE it_header INTO wa_header INDEX 1.
*IF sy-subrc = 0.
*lv_cd = wa_header-ZBUKR.
*lv_comp = wa_header-FULL_NAME.
*
*ENDIF.
*
